...
public class Ex01 {
	...
	// 제네릭: 컬렉션이 어떤 객체들로 이루어졌는지를 표시하는 객체 타입을 말한다.
	// 컬렉션: 객체들을 모아서 관리하는 인터페이스를 구현한 것
	// API : <T(ype)> => 객체 타입, <E(lements)> => 구성 요소(컬렉션 안에 존재하는 객체 하나를 뜻한다.)
	// Map형식에서는 <K(ye),V(alue)> => Key와 Value가 1:1대응, Kye를 호출하면 Value가 나온다
	// Map형식에서는 <K(ey),V(alue)> => Key와 Value가 1:1대응, Kye를 호출하면 Value가 나온다
	// 형식 : 컬렉션<제네릭>
	// 최상위 컬렉션은 Collection<E>, Map<K,V>

	// Collection<E>를 상속받은 Interface : List<E>, Set<E>, Queue<E>
