...
public class Ex02 {
	public static void main(String[] args) {
		...
		// Calendar 클래스
		// 현재 날짜와 시간을 구할 때는 new예약어를 사용하지 않고 객체생성한다.
		// 년,월,일,시,분,초를 구할 때 Calendar.getInstance().get(Calendar.상수) 사용
		System.out.println(Calendar.getInstance().get(Calendar.YEAR)+"년도1");
		System.out.println();
		Calendar now = Calendar.getInstance(); // 컴퓨터의 현재 날짜와 시간, 요일을 뜻한다.
		System.out.println(now.get(Calendar.YEAR)+"년도2");
		System.out.println(now.get(Calendar.MONTH)+1+"월");
		System.out.println(now.get(Calendar.DATE) + "일1");
		System.out.println(now.get(Calendar.DAY_OF_MONTH) + "일2");
		
		// 시간: 12시간제, 24시간제
		System.out.println(now.get(Calendar.HOUR)+"시1"); //12시간
		System.out.println(now.get(Calendar.HOUR_OF_DAY)+"시2"); //24시간
